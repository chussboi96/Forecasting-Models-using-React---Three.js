...
const ANN = () => {
  ...
  useEffect(() => {
    d3.csv('/last_3_months.csv').then(data => {
      ...
      // Filter data for the full period (last 3 months)
  ...
  const updateData = (data, range) => {
    let filtered = [];
    const now = new Date(data[0].timestamp); // Assuming the start of the dataset
  ...
  return (
    <div className='ann-body'>
      <div style={{ marginBottom: '10px' }}>
        ...
        <button onClick={() => updateData(data, '3months')}>3 Months</button>
      </div>
      <svg ref={svgRef}></svg>
    </div>
  );
